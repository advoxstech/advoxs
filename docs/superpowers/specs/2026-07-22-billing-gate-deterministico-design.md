...
Essa arquitetura torna o gate de billing que hoje vive em `apps/agents` (tool + checagem dentro do `agent_node`) redundante — ele é removido: a garantia "só chega no `agents` quem tem saldo" passa a ser estrutural (decidida antes, no `worker`), não mais uma checagem que o LLM/tool precisa fazer por conta própria.
...
1. **Entrada no gate**: a cada mensagem nova, se `state == agent`, cobrança habilitada e `end_customer_balances.credit_balance <= 0` → `state = billing_gate`, `billing_gate_step = None`.
...
- **Conflito com o commit `0e8267e`** (mergeado em paralelo a este design, no mesmo tenant de billing do cliente final): esse commit fez o webhook de pagamento enfileirar uma mensagem de sistema pro `process_inbound_message`, especificamente pra fazer o agente "saber" que o pagamento foi confirmado e completar a transferência sozinho. Decisão: esse mecanismo é **revertido** como parte deste design — a transição de estado direta (`billing_gate → agent`, ver "Fluxo" acima) resolve o mesmo problema sem precisar acionar o `agents`, e mantê-lo ativo reintroduziria exatamente o custo de LLM que este design existe pra eliminar nesse ponto.
- **Trabalho da mesma sessão que se torna obsoleto**: o aviso fixo de retorno ao ponto de entrada por saldo esgotado (`bounced_from_billing_block`) e a instrução de prompt que impede revelar `package_id`/exige colar o link literal (ambos em `apps/agents/agents/nodes.py`) vivem inteiramente dentro do bloco de billing que este design remove do `agents`. Decisão: aceito como consequência — não há mais "secretária oferecendo pacotes" pra precisar dessas instruções, o fluxo de saldo esgotado passa a ser 100% do `worker`/WhatsApp nativo.
...
- Feature flag por tenant pra rollout gradual — ver "Riscos" abaixo; decidir separadamente se o corte é único ou gradual antes de implementar.
- Criar o enum `insufficient_balance_policy` mencionado em specs anteriores — `billing_gate` como `state` já cobre o comportamento de `block_with_message`; formalizar a doc só depois de implementado, se ainda fizer sentido.
...
- **Corte único, sem feature flag**: é uma mudança em um caminho de billing já em produção. Vale decidir explicitamente (fora deste documento) se o rollout é de uma vez ou tenant por tenant antes de escrever o plano de implementação.
