...
- ✅ **Fundação da wallet unificada (Etapa 1, migration `0013`) implementada — sem mudança de comportamento**: tabela global versionada `pricing_configs` (seed: 1000 tokens/crédito, `input_weight` 0.3, `output_weight` 1.0; leitura via `get_current_pricing_config` em `apps/api/app/services/pricing.py`, ainda sem consumidor), saldos e `amount_credits` dos dois ledgers em `Numeric(12,4)` (créditos fracionados), tipos `resale`/`adjustment` aceitos nos checks dos ledgers, e colunas de auditoria `tokens_input`/`tokens_output`/`pricing_config_id` nos dois ledgers — o `agents` devolve `tokens_input`/`tokens_output` (além de `tokens_used`, mantido) em `/messages` e `/summaries`, e `worker`/`api` já gravam os tokens brutos em cada lançamento de consumo. A fórmula de débito (`ceil` na env) **continua a mesma** — a ponderação input/output e o modelo de moeda única (revenda `resale` debitando o estoque do tenant) são a Etapa 2/3 do plano em `docs/superpowers/plans/2026-07-17-etapa-1-fundacao-creditos.md`.
- ✅ **Bloqueio por saldo esgotado implementado**: o `worker` (`process_inbound_message`) consulta `credit_balance` do tenant antes de chamar o `agents` service — quando `credit_balance <= 0`, a chamada é pulada (nenhum débito, nenhum custo de LLM) e o cliente final recebe **silêncio total**: a mensagem já persistida pelo `api` fica apenas visível em `/conversas`, esperando um humano do escritório assumir (mesmo comportamento do modo `human`). O painel (`web`) mostra um banner de aviso (`LowBalanceBanner`, reaproveitando `GET /api/v1/billing/balance`) nas páginas do tenant (`/conversas`, `/base-de-conhecimento`, `/configuracoes/whatsapp`) com link pra `/creditos`; omitido na própria `/creditos` (redundante com o saldo já exibido ali). Sem buffer de tolerância — o limite é exatamente `<= 0`.
- O resumo de conversa sob demanda também debita créditos pela mesma fórmula (`ceil(tokens/CREDIT_TOKENS_PER_CREDIT)`), com `related_message_id=None` — reaproveita a env já existente, mas agora replicada em `apps/api/app/core/config.py` (o `api`, não só o `worker`, também converte tokens em créditos, para a rota de resumo).
