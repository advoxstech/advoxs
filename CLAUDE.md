...
- **Gate técnico no grafo do `agents`**: sem saldo (feature habilitada + `balance <= 0`), a tool `transfer_to_agent` recusa a transferência e o ponto de entrada oferece os pacotes cadastrados/gera o link em vez de transferir. Esse saldo é **re-checado a cada turno dentro do `agent_node` genérico também**, não só na transferência inicial — sem isso, uma vez transferida a conversa fica fixada no agente de destino (`current_agent_id` no checkpoint), então um cliente que comprasse um pacote pequeno ganharia atendimento gratuito ilimitado depois de esgotar o saldo. Quando bloqueado, o agente que não é o ponto de entrada é atendido pelo próprio ponto de entrada no mesmo turno, em vez de responder (ver seção Agents Service).
...
- **CRUD** (`/api/v1/agents`, autenticado e tenant-scoped): `GET` lista os agentes do tenant; `POST` cria (`{name, instructions, is_entry_point}`); `PATCH /{agent_id}` atualiza campos parciais; `DELETE /{agent_id}` apaga — ambos recusam (`409`) desmarcar/apagar o **único** ponto de entrada ou apagar o **último** agente restante do tenant (todo tenant precisa de ao menos 1 agente, e exatamente 1 marcado `is_entry_point`). Promover um novo ponto de entrada (`PATCH {is_entry_point: true}`) desmarca automaticamente o anterior (`_unset_current_entry_point`) — nunca fica sem nenhum ou com mais de um.
...
- Pendência: seletor de "compartilhar arquivo já anexado com outro agente" direto da tela `/agentes/{id}` de outro agente (a API já suporta N:N — `POST /agents/{id}/knowledge-base-files` aceita qualquer `knowledge_base_file_id` do tenant — falta só o atalho de UX pra escolher entre os arquivos de outro agente, hoje o seletor já filtra certo dentro do mesmo agente).
