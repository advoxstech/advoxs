...
- ✅ **Cobrança do cliente final** (ver seção Billing / Créditos): tool `gerar_link_pagamento_cliente` (bindada a todo agente quando a cobrança está habilitada pro tenant) chama um endpoint interno do `api` pra gerar o link de pagamento — nunca vê a secret key da Stripe do tenant. Gate técnico dentro do `agent_node` genérico (`is_billing_blocked`, avaliado uma única vez por execução, sem duplicação entre nós): sem saldo, um agente que não é o ponto de entrada é atendido pelo ponto de entrada no mesmo turno (sem custo de LLM extra), que oferece os pacotes cadastrados pelo tenant; `transfer_to_agent` também recusa a transferência nesse caso. O saldo/pacotes chegam no `state["end_customer_billing"]` (propagado desde o `POST /messages`) e são injetados nos argumentos da tool pelo `tool_node` — nunca confiando em valor que o LLM tente passar.
  - ✅ **Aviso de retorno ao ponto de entrada**: quando essa transição acontece (`billing_blocked and not current["is_entry_point"]` em `agent_node`), uma `AIMessage` fixa e programática (não gerada pelo LLM) é injetada antes da resposta normal do ponto de entrada: `"voltando para {nome do ponto de entrada} — o atendimento anterior ficou indisponível porque os créditos acabaram."`. Dispara exatamente uma vez por bloqueio — `current_agent_id` já é persistido como o do ponto de entrada no mesmo turno, então a condição nunca volta a ser verdadeira até uma nova transferência-e-bloqueio.
  - ⚠️ **Instruções do prompt de billing** (mesmo bloco de `agent_node` que injeta a oferta de pacotes): o modelo é explicitamente instruído a nunca mencionar o `package_id` ao cliente (é só um identificador interno pra chamar a tool) e a copiar o link de pagamento devolvido pela tool literalmente na resposta — bug real encontrado em produção (a secretária citava o `package_id`, um uuid, e às vezes dizia "gerei o link" sem colar a URL de verdade), corrigido só com texto de prompt, sem mudança de tool/lógica.
