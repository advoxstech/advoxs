...
Front (`web`): `/admin/login`, `/admin` (dashboard, stat tiles + gráfico de novos escritórios), `/admin/tenants` (lista), `/admin/tenants/[id]` (detalhe) — nav lateral própria, middleware com bloco isolado para `/admin/*` (nunca compartilhado com a lógica de sessão de tenant).
