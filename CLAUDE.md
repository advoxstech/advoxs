...
- ✅ **Base de conhecimento agrupada por agente (2026-07-22)**: `/base-de-conhecimento` deixou de ter um `<select>` de agente de destino no topo — virou uma árvore expansível, 1 pasta por agente (`AgentFolder.tsx`, incluindo agentes com 0 arquivos), com upload já escopado por pasta (cada pasta manda seu próprio `agent_id`, sempre explícito). Um arquivo em mais de 1 agente aparece em cada pasta correspondente (N:N preservado). Cada arquivo tem 3 ações: anexar a outro agente (resolve a antiga pendência de "compartilhar com outro agente" citada acima), desanexar deste agente (bloqueado com `409` se for o último vínculo — `DELETE /agents/{id}/knowledge-base-files/{file_id}` agora recusa deixar um arquivo "sem agente", já que isso o tornava invisível pra qualquer busca de RAG) e excluir o arquivo. `GET /knowledge-base/files` ganhou o campo `agent_ids: list[uuid.UUID]` por arquivo (join com `agent_knowledge_base_files`, sem migração) — é o dado que a árvore usa pra se montar. `/agentes/{id}` não gerencia mais KB (ver acima) — só mostra a contagem + um link que abre a pasta certa. Spec/plano: `docs/superpowers/specs/2026-07-21-kb-agrupada-por-agente-design.md` / `docs/superpowers/plans/2026-07-21-kb-agrupada-por-agente.md`.
