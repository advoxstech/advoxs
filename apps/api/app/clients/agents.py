...
async def send_playground_message(
    *, tenant_id: str, contact_phone_number: str, message: str, agents: list[dict] | None = None
    ...
    "documents": [...]}, ou None quando o agents devolve 202 (debounce
    agrupou a mensagem numa execução em andamento — as respostas virão pela
    execução que já roda). `documents`: documentos gerados nesta execução
    (fazer_contrato/fazer_multa/etc), cada item com {"link", "filename",
    "credit_cost", "delivered"} — `delivered` sempre False aqui, já que o
    playground/conversa de teste chama com send_to_whatsapp=False.
